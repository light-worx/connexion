...
class ManageRoster extends Page implements HasForms
{
    ...
    protected string $view = 'filament.pages.manage-roster';
    ...
    protected function constructPreviewSchema(){
        ...
        $schema=[TextEntry::make('PreviewDate')->state(new HtmlString('<b>' . date('l d F Y',strtotime($rosterdate)) . '</b>'))->label('')];
        ...
        $schema[] = TextEntry::make('Bulksms Credits')->label('')->state(function (){
        ...
        $schema[] = TextEntry::make('Preview')->state(new HtmlString($messages))->label('');
        return $schema;
    }
    ...
    protected function getWeeks($firstofmonth)
    {
        ...
        for ($j=1;$j<=4;$j++){
            if ((count($weeks)) && ($thismonth==date('Y-m',strtotime($weeks[0] . ' + ' . $j . ' week')))){
    ...
    public function form(Schema $form): Schema
    {
        ...
        foreach ($weeks as $ndx=>$label){
            if ($ndx==0){
                $schema[] = TextEntry::make('blank')->hiddenLabel(true);
            }
        ...
        foreach ($rostergroups as $ndx=>$rg) {
            $this->data['rgs'][]=$rg->id;
            $schema[] = TextEntry::make($rg->group->groupname)->state($rg->group->groupname)->hiddenLabel(true);
            $members=[];
