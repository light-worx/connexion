...
@php
    ...
    $rosterId        = $timeSettings['roster_id'] ?? null;

    $date  = $plan->sundayGroup->service_date->toDateString();
    $time  = $plan->service_time;

    $rosterUrl = $rosterId
        ? route('filament.admin.people.resources.rosters.manage', [
            'record'      => $rosterId,
            'rostermonth' => $date,
          ])
        : (\Illuminate\Support\Facades\Route::has('filament.admin.people.resources.rosters.index')
        ...
        @foreach ($rosterItems as $rosteritem)
            <div class="rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 px-4 py-3">
                <p class="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-2">
                    {{ $rosteritem->rostergroup->group->groupname ?? '—' }}
                </p>
                @forelse ($rosteritem->individuals ?? [] as $individual)
                    <div class="flex items-center gap-2 py-1">
                        ...
                        <span class="text-sm text-gray-700 dark:text-gray-300">
                            {{ trim(($individual->firstname ?? '') . ' ' . ($individual->surname ?? '')) ?: '—' }}
                        </span>
                    </div>
                @empty
